class Dropdown {
  constructor(dropdownId, selectedColorId = "sColor1", greenId = "razerGreen1") {
  ...
  getDropdownMode(e) {
    ...
    if (e.target.classList.contains("option") || e.target.classList.contains("preset")) {
  ...
  toggleDropdown(e, forceClose = false) {
    if (e.target.classList.contains("option") && e.target.parentElement.id != this.dropdownId + "Opt") return;
    if (forceClose) {
      this.open = true;
    }
    if (!this.open) {
  ...
  handleDropdown(e) {
    ...
    if (mode == "select" && this.optDom.classList.contains("color-opts") && isRight) {
      e.preventDefault();
